...
public class Box extends Shape {
  ...
  public Point2D[] getRotatedPoints(float radians) {
    Point2D[] points = getPoints();
    for (int i = 0; i < 4; i++) {
      Point2D point = points[i];
      float tX = point.x - center.x;
      float tY = point.y - center.y;
      float rotatedX = tX * cos(radians) - tY * sin(radians);
      float rotatedY = tX * sin(radians) - tY * cos(radians);
      float x = rotatedX + center.x;
      float y = rotatedY + center.y;
      point.x = x;
      point.y = y;
    }    
  ...
  public Point2D[] getPoints() {
    Point2D pointA = new Point2D(getMinX(), getMinY());
    Point2D pointB = new Point2D(getMinX(), getMaxY());
    Point2D pointC = new Point2D(getMaxX(), getMaxY());
    Point2D pointD = new Point2D(getMaxX(), getMinY());
